...
function printProducts(products){
  ...
  let sections = getFromLocalStorage('sectionsProducts');
  let productsToShow = products.slice(0, currentProductsLimit);
  ...
  renderLoadMore(products.length);
}
function renderLoadMore(totalProducts){
  let container = document.querySelector('#load-more-container');
  if(!container) return;
  let buttons = '';
  if(currentProductsLimit > 12 && totalProducts > 12){
    buttons += `<button id="show-less-btn">Show Less</button>`;
  }
  if(currentProductsLimit < totalProducts){
    let remaining = totalProducts - currentProductsLimit;
    buttons += `<button id="load-more-btn">Load More (${remaining} remaining)</button>`;
  }
  ...
  if(loadMoreBtn){
    loadMoreBtn.addEventListener('click', function(){
      ...
      printProducts(currentFilteredProducts);
      getButtonsForAdding();
      storeSingleProductToLS();
      getClickedModal();
    });
  ...
  if(showLessBtn){
    showLessBtn.addEventListener('click', function(){
      ...
      storeSingleProductToLS();
      getClickedModal();
      window.scrollTo({top: document.querySelector('.aa-product-catg').offsetTop - 100, behavior: 'smooth'});
    });
  }
}
...
window.onload= function(){
  ...
  setTimeout(function(){
    ...
    let paramGender = urlParams.get('gender');
    let paramCategory = urlParams.get('category');
    let filtersApplied = false;
    if(paramGender){
      genders.forEach(gender => {
        if(gender.value == paramGender){
          gender.checked = true;
          filtersApplied = true;
        }
      });
    ...
    if(paramCategory){
      categories.forEach(category => {
        if(category.value == paramCategory){
    ...
    getDataWithAjax('products.json', function(products){
      ...
      products.forEach(product => {
        ...
        if(filteredProducts.length === 0){
          relatedProductsWrapper.innerHTML = '<li style="list-style:none;text-align:center;width:100%;padding:5px 0 30px;color:#555;height:auto;margin:0;">No related products found for this item.</li>';
        } else {
